...
export default function FollowUps() {
  ...
  return <DashboardShell>
    ...
    <p className="mt-1 text-sm text-slate-600">{role === 'patient' ? 'Your upcoming scheduled follow-up care (read-only).' : 'Upcoming, due, completed and missed follow-up care.'}</p>
    ...
    <section className="card mt-5 overflow-x-auto">
      <table className="w-full min-w-[640px] text-left text-sm">
        <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Patient</th><th className="p-4">Date</th><th className="p-4">Reason</th><th className="p-4">Status</th>{canManage && <th className="p-4">Action</th>}</tr></thead>
        <tbody>{rows.map(row => <tr className="border-t" key={row.id}>
          ...
          {canManage && <td className="p-4">{row.status !== 'completed' && <button disabled={busyId === row.id} onClick={() => complete(row)} className="inline-flex items-center gap-1 text-xs font-bold text-blue-700 disabled:opacity-50"><CheckCircle2 className="h-4 w-4" />{busyId === row.id ? 'Saving...' : 'Complete'}</button>}</td>}
        </tr>)}</tbody>
      </table>
    </section>
  </DashboardShell>;
