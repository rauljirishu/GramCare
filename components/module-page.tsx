...
export function ModulePage({ module }: {module:string}) {
 ...
 return <DashboardShell><div className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">GramSwasthya · Demo workspace</p><h1 className="mt-2 text-3xl font-black text-slate-900">{data.title}</h1><p className="mt-1 text-slate-600">{data.subtitle}</p></div><button className="primary-btn" onClick={()=>setNotice(`${action} form opened in prototype mode. Your action is recorded locally for this demo.`)}><Plus className="h-4 w-4"/>{action}</button></div>
 ...
 {module==='map'&&<button className="secondary-btn mb-5" onClick={()=>navigator.geolocation?.getCurrentPosition(()=>setNotice('Location permission granted. Demo locations are displayed.'),()=>setNotice('Location access was not granted. Search manually or select a listed facility.'))}><MapPin className="h-4 w-4"/>Use my location</button>}
 ...
 <p className="mt-4 flex items-center gap-2 text-xs text-slate-500"><WifiOff className="h-4 w-4"/>Demo data is stored locally when offline; actual backend sync requires configured secure services.</p></DashboardShell>
}
